...
기본 API process가 자동 turn 정리 loop를 실행합니다. 기본값은 활성화, 60분 간격,
transaction batch당 최대 100개입니다.
...
interval 범위는 1~1440분, batch 범위는 1~1000개이며 범위를 벗어나면 startup
validation이 실패합니다. 삭제에 성공하면 기존 audit event를 보존하고
metadata-only `turn_summary.retention.pruned`를 추가합니다. 정리 실패는 log에 남기고
API를 종료하지 않은 채 다음 interval에 다시 시도합니다. 외부 공개 승인·취소,
