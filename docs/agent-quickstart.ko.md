...
알고 있는 경우 정규화된 비민감 `projectKey`, `taskKey`, `topicTags`를 보낼 수
있습니다. 프로젝트 범위 조회는 같은 프로젝트와 전역 기록을 포함하고 다른
프로젝트 기록을 제외한 뒤 작업·주제·최신성 점수를 제한적으로 적용합니다. 원본
...
에이전트용 connector는 기존 memory·source·turn 기록을 변경하거나 삭제할 수
없도록 설계되어 있으며 민감데이터 접근을 승인하거나 거부할 수도 없습니다.
에이전트나 사용자가 삭제·수정·덮어쓰기·승인·거부를 요청하면 에이전트는 명시적으로
거절하고 도구를 만들어내거나 운영자 경로를 호출해서는 안 됩니다. 그런 결정은
신뢰된 운영자 또는 제한된 시스템 보존 정리가 담당하며 에이전트 service token의
권한이 아닙니다.

## Hook이 수집하는 내용

Stop hook은 최종 assistant 응답에서 만든 제한된 capsule과 중복 방지를 위한 hash 식별자만 보냅니다. 전체 대화 기록, 사용자 prompt, 작업 폴더, 대화 기록 경로, 자격 증명 파일, Luthn 서비스 token은 읽거나 올리지 않습니다. macOS와 Linux 전송은 비동기이며 작업 완료를 막지 않습니다. Windows에서는 Codex가 분리된 업로더를 종료하지 못하도록 훅이 제한된 업로드를 기다리며, 훅 제한 시간은 10초이고 실패 허용 동작은 유지됩니다. 자주 쓰이는 자격 증명 모양을 로컬에서 가리고 모든 capsule을 분류한 뒤에만 공유 맥락으로 만들 수 있습니다.
...
원본 Vault 일괄 추출, 제한 없는 원본 조회, 비공개 기록 내보내기는 기본 에이전트 기능이 아닙니다. connector는 위 세 가지 메타데이터 전용 요청 도구에 필요한 `access.request`와 제한된 cache 관측·명시적 feedback용 `metrics.write` scope를 기본 설정하지만 MCP는 승인·거절을 노출하지 않습니다. 비공개 세부 정보 결정에는 별도의 신뢰된 운영자 경로가 필요합니다. `submit_search_feedback`은 Luthn이 반환한 `retrievalId`와 `helpful|unhelpful`만 받고 query, 결과 본문, 자유형 의견은 받지 않습니다. 검색 지표 보고 실패는 회상 결과나 timeout/cache 동작을 바꾸지 않습니다.
