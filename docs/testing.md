...
The canonical inventory currently contains 59 C# test sources, 7 .NET test
projects, and 17 files under `scripts/tests/`, for 83 test-related files. The
latest full .NET run covered 653 test cases. File count and test-case count are
...
| `fast` | `bash scripts/tests/test-public-source-boundary.sh` | Reject private managed-service namespaces, routes, contracts, vendors, and origins from the public source tree. |
...
| `scripts/tests/test-public-source-boundary.sh` | `fast` | `bash scripts/tests/test-public-source-boundary.sh` | Public/private source boundary for managed-service implementation details. |
...
| `tests/Luthn.Host.Api.Tests/HostManagedExtensionTests.cs` | `focused` | `dotnet test tests/Luthn.Host.Api.Tests/Luthn.Host.Api.Tests.csproj --no-restore --filter FullyQualifiedName~HostManagedExtensionTests` | Provider-neutral signed extension provisioning, local approval, cleanup, and secret suppression. |
| `tests/Luthn.Host.Api.Tests/HostMcpProfileTests.cs` | `focused` | `dotnet test tests/Luthn.Host.Api.Tests/Luthn.Host.Api.Tests.csproj --no-restore --filter FullyQualifiedName~HostMcpProfileTests` | Explicit local/remote MCP profile action, observation, authorization, and rollback contracts. |
